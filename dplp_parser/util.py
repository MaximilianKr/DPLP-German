...
# ( SARA
def is_free_form(vocab, dpvocab, projmat):
    ndpvocab = len(dpvocab)
    if projmat.shape[0] == ndpvocab:
        return False
    elif projmat.shape[0] == 3 * ndpvocab + len(vocab):
        return True
    else:
        assert "Number of projection matrix rows {} doesn't match the feature count".format(projmat.shape[0])
# SARA )
...
def vectorize(features, vocab, dpvocab=None, projmat=None):
    ...
    if withdp:
        ...
        vec = hstack([normalize(vec), vec_dense])

...
from dplp_parser.featselection import FeatureSelection
...
class Data(object):
    ...
    def buildmatrix(self):
        ...
        if self.withdp:
            ...
            if is_free_form(self.vocab, self.dpvocab, self.projmat):
                self.nC = nlatent
            else:
                self.nC += (3 * nlatent)
        # SARA )
        ...
        for (sidx, sample) in enumerate(self.samplelist):
            ...
            vec = vectorize(sample, self.vocab, self.dpvocab, self.projmat)
            self.L.append(lidx)
            vec = lil_matrix(vec)
